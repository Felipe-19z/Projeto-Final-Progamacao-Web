...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$nome || !$email || !$descricao) {
        ...
        // Evitar duplicação acidental: checar se já existe mensagem igual enviada nos últimos 5 segundos
        $checkSql = "SELECT id FROM mensagens_ajuda WHERE usuario_id = ? AND nome = ? AND email = ? AND descricao = ? AND data_criacao >= (NOW() - INTERVAL 5 SECOND) LIMIT 1";
        $checkStmt = $conn->prepare($checkSql);
        if ($checkStmt) {
            $checkStmt->bind_param("isss", $usuario_id, $nome, $email, $descricao);
            $checkStmt->execute();
            $checkStmt->store_result();
            if ($checkStmt->num_rows > 0) {
                // Já foi enviada recentemente — evitar inserir novamente
                $sucesso = 'Mensagem já enviada (duplicada evitada). Obrigado.';
                $checkStmt->close();
            } else {
                $checkStmt->close();
            ...
            // Se falhar a checagem, prosseguir com inserção para não bloquear o usuário
            $sql = "INSERT INTO mensagens_ajuda (usuario_id, nome, email, descricao) VALUES (?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("isss", $usuario_id, $nome, $email, $descricao);
            $stmt->execute();
