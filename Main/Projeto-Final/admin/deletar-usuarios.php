...
// id do admin que está realizando a exclusão
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Registrar na auditoria — excluido_por referencia `usuarios(id)`, portanto
        // não podemos inserir o id do admin (que está em tabela `admin`). Para
        // respeitar a FK, gravamos NULL em `excluido_por` e mantemos nome/email/motivo.
        $sql = "INSERT INTO auditoria_exclusao (usuario_id, nome_usuario, email_usuario, motivo_exclusao, excluido_por)
            VALUES (?, ?, ?, ?, NULL)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("isss", $id, $usuario['nome'], $usuario['email'], $motivo);
        $stmt->execute();
        $stmt->close();
        ...
        <div class="card">
            ...
            <form method="POST">
                ...
                <div class="button-group">
                    ...
                    <button type="submit" class="btn-delete" onclick="return confirm('ATENÇÃO: Esta ação é irreversível. Tem certeza que deseja deletar este usuário?')">
                        Deletar Usuário
                    </button>
                </div>
            </form>
