...
$sql_check = "SELECT id, nome, email, senha, ativo, is_admin, is_premium, premium_plan FROM usuarios WHERE email = ?";
...
$sql = "SELECT id, nome, email, senha, ativo, is_admin, is_premium, premium_plan FROM usuarios WHERE email = ? AND ativo = TRUE";
...
// Definir flag is_premium na sessão
if (isset($usuario['is_premium'])) {
    $_SESSION['is_premium'] = (bool)$usuario['is_premium'];
    $_SESSION['premium_plan'] = $usuario['premium_plan'] ?? null;
} else {
