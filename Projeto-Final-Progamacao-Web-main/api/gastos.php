...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'criar') {
        ...
        // tipo_gasto: 'opcional' (default) or 'obrigatorio'
        $tipo_gasto = (isset($data['tipo_gasto']) && $data['tipo_gasto'] === 'obrigatorio') ? 'obrigatorio' : 'opcional';
        ...
        $sqlCreateFix = "CREATE TABLE IF NOT EXISTS gastos_fixos (
            ...
            valor DECIMAL(10,2) NOT NULL,
            tipo_gasto ENUM('opcional','obrigatorio') DEFAULT 'opcional',
        ...
        // Garantir compatibilidade: se a tabela gastos existe sem a coluna tipo_gasto, tentar adicionar a coluna.
        try {
            $colCheck2 = $conn->query("SHOW COLUMNS FROM gastos LIKE 'tipo_gasto'");
            if ($colCheck2 && $colCheck2->num_rows === 0) {
                $conn->query("ALTER TABLE gastos ADD COLUMN tipo_gasto ENUM('opcional','obrigatorio') DEFAULT 'opcional'");
            }
        } catch (Exception $e) {
            error_log('gastos.php: falha ao garantir coluna tipo_gasto: ' . $e->getMessage());
        }

        // Rechecar se a coluna foi realmente criada/está disponível.
        $colCheck2b = $conn->query("SHOW COLUMNS FROM gastos LIKE 'tipo_gasto'");
        $gastos_has_tipo = ($colCheck2b && $colCheck2b->num_rows > 0);
        ...
        if ($is_fixo) {
            ...
            $sqlInsFix = "INSERT INTO gastos_fixos (usuario_id, categoria_id, descricao, valor, tipo_gasto, periodicidade, start_date, periodos) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            $stmtFix = $conn->prepare($sqlInsFix);
            if ($stmtFix) {
                $stmtFix->bind_param('iisdsssi', $usuario_id, $categoria_id, $descricao, $valor, $tipo_gasto, $periodicidade, $start_date, $periodos);
                $stmtFix->execute();
        ...
        if ($resFind && $resFind->num_rows > 0) {
            ...
            // when merging, keep existing tipo_gasto unless new one is 'obrigatorio'
            $tipo_to_store = $tipo_gasto;
            if ($gastos_has_tipo) {
                $sqlGetTipo = "SELECT tipo_gasto FROM gastos WHERE id = ? LIMIT 1";
                $stmtTipo = $conn->prepare($sqlGetTipo);
                if ($stmtTipo) {
                    $stmtTipo->bind_param('i', $existing_id);
                    $stmtTipo->execute();
                    $resTipo = $stmtTipo->get_result();
                    if ($resTipo && $resTipo->num_rows > 0) {
                        $rowTipo = $resTipo->fetch_assoc();
                        if ($rowTipo['tipo_gasto'] === 'obrigatorio') $tipo_to_store = 'obrigatorio';
                    }
                    $stmtTipo->close();
                }
            }

            // Atualizar gasto existente — usar versão que inclui tipo_gasto somente se a coluna existir
            if ($gastos_has_tipo) {
                $sqlUpdate = "UPDATE gastos SET valor = ?, hora_gasto = ?, descricao = ?, tipo_gasto = ? WHERE id = ?";
                $stmtUpd = $conn->prepare($sqlUpdate);
                $shortDesc = substr($descricao, 0, 255);
                if ($stmtUpd) {
                    $stmtUpd->bind_param("dsssi", $new_total, $hora_gasto, $shortDesc, $tipo_to_store, $existing_id);
                    $stmtUpd->execute();
                    $stmtUpd->close();
                }
            } else {
        ...
        // Inserir novo gasto: usar coluna tipo_gasto apenas se disponível no schema
        if ($gastos_has_tipo) {
            $sql = "INSERT INTO gastos (usuario_id, categoria_id, descricao, valor, tipo_gasto, data_gasto, hora_gasto) 
                VALUES (?, ?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
            if ($stmt) {
                $stmt->bind_param("iisdsss", $usuario_id, $categoria_id, $descricao, $valor, $tipo_gasto, $data_gasto, $hora_gasto);
                $stmt->execute();
                $gasto_id = $stmt->insert_id;
                $stmt->close();
            } else {
                api_json(['success' => false, 'message' => 'Erro ao preparar insert de gasto: ' . $conn->error]);
            }
        } else {
            ...
            $stmt = $conn->prepare($sql);
            if ($stmt) {
                $stmt->bind_param("iisdss", $usuario_id, $categoria_id, $descricao, $valor, $data_gasto, $hora_gasto);
                $stmt->execute();
                $gasto_id = $stmt->insert_id;
                $stmt->close();
            } else {
                api_json(['success' => false, 'message' => 'Erro ao preparar insert de gasto (sem tipo_gasto): ' . $conn->error]);
            }
        }
