...
<body>
    ...
    <script>
        ...
        document.getElementById('btnDeleteUser').addEventListener('click', async function() {
            // If deletion is disabled due to being last admin, do nothing
            if (this.disabled) return;
            const ok = await showConfirm('ATENÇÃO: Esta ação é irreversível. Tem certeza que deseja deletar este usuário?', 'Deletar Usuário');
            if (!ok) return;
            // submit the form
